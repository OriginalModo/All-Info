...
Суть: Отделяет абстракцию от ее реализации так, чтобы и то и другое можно было изменять независимо


В каких случаях рекомендуется использовать:
1) изменения абстракции и реализации не должны сказываеться на клиентском коде(клинета)
2) при разделении обьемного класа, содержащего несколько различных реализаций некоторой функциональности
3) необходимо избежать постоянной привязки абстракции к реализации

Плюсы:
--- сокрытие деталей реализации обьекта от клиентского кода
--- разработки платформо-независимых программ
--- возможность динамически менять реализацию в процессе работы программы
...
class ClassicIOvenImplementor(IOvenImplementor):
...
class ElectricalIOvenImplementor(IOvenImplementor):
...
if __name__ == '__main__':
    ...
    implementor = ClassicIOvenImplementor()
    ...
    # замена реализации
    new_implementor = ElectricalIOvenImplementor(oven.get_temperature())

...
Суть: позволяет выделить запрос в виде отдельного конфигурируемого обьекта который может передаваться как аргументам
при вызове методов так и ставиться в очередь на исполнение или логирование а также поддерживает отмену операций

В каких случаях рекомендуется использовать:
1) необходимо осуществить параметризацию обьектов выполняемым действием
2) запросы должны организовываться в очереди или выполняться по расписанию
3) важно иметь возможность поддерживать отмену операций или протоколирование изменений

Плюсы:
--- легко организуются операции отмены, повторения и отложенный запуск операций
--- можно собрать сложные команды из простых

Минусы:
--- заводится большое количество дополнительных классов
"""

...
Суть: для сохранения текущего и восстановления прерыдущих снимков состояния обьектов без раскрытия подробно их реализации

В каких случаях рекомендуется использовать:
1) необходимо сохранять мгновенные снимки состояния обьекта, которые можно использовать для восстановления его
сохраненного состояния
2) если прямое получение состояния обьекта нарушает инкапсуляцию, тем самым раскрывая детали его реализации

Плюсы:
--- упрощает структуру исходного обьекта, путем отделения от неё истории его состояний и не нарушает его инкапсуляцию

Минусы:
--- при создании большого количества снимков состояний исходного обьекта требуется много памяти
"""

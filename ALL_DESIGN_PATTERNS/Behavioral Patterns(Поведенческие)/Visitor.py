...
Суть: позволяет определять новую операцию для обьектов не внося изменения в их классах

В каких случаях рекомендуется использовать:
1) необходимо выполнить операции, над множеством обьектов с различными интерфейсами
2) необходимо выполнять не связанные между собой операции над обьектами, входящими в состав структуры данных
3) операции над обьектами добавляются намного чаще, чем производится изменение классов, описывающих их структуру

Плюсы:
--- упрощает механизм добавления новых операций
--- позволяет обьединить все родственные операции в одном классе
--- позволяет накапливать состояние в процессе обхода структуры элементов

Минусы:
--- вероятность нарушения инкапсуляции элементов
--- неоправданность применения этого паттерна проектирования при частом изменении иерархии элементов
"""

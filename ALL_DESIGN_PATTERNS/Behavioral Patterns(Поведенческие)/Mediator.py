...
Суть: чтобы организовать способ взаимодействия между множеством обьектов, уменьшая тем самым связность между ними
...
3) необходимо конфигурировать и распределить поведение между классами, без порождения множества производных классов

Плюсы:
--- позволяет централизовать управление и снизить число производных классов
--- упрощает взаимодействие между компонентами и устраняет зависимости между ними

Минусы:
--- может сильно усложнить (раздуться) код паттерного класса
"""
...
class IMediator(ABC):
    ...
    @abstractmethod
    def notify(self, worker: T, order: Order, event: Event):
        ...

    @abstractmethod
    def add_worker(self, worker: T) -> None:
        ...

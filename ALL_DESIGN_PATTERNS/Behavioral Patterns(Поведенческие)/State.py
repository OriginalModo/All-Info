...
Суть: позволяет обьекту варьировать своё поведение в зависимости от его внутреннего состояния, создавая впечатление
что изменяеться сам класс этого обьекта

В каких случаях рекомендуется использовать:
1) поведение обьекта должно зависить от его состояния
2) наличие множества условных ветвлений и выбор ветви зависит от состояния обьекта
3) если планируется использовать иерархическую машину состояний, в основе которой лежит наследование

Плюсы:
--- явные переходы между состояниями
--- код связанный с тем или иным состоянием концентрируется в определенном месте
--- очистка кода от множества условных переходов машины состояния

Минусы:
--- усложнение кода при малом количестве состояний и их редком изменении
"""

...
Суть: предоставляет обьектам в системе возможность подписыватся на изменение состояния или события которые происходят в
других обьектах

В каких случаях рекомендуется использовать:
1) необходимо при модификации одного обьекта произвести изменения в других и нет понимания о каком количестве может идти речь
2) необходимо в определенных случаях наблюдать за изменениями ряда обьектов в разрабатываемой системе

Плюсы:
--- позволяет одним обьектам 'на лету' подписываться и отписываться от событий другого обьекта
--- обьекты, выступающие в качестве подписчиков, не зависят от конкретных классов обьектов, генерирующих события и наоборот

Минусы:
--- оповещение обьектов производится в случайном порядке
"""
